...
#include "../../features/combat/modules/silent_threads.h"
...
void overlay::load_interface()
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowExW(WS_EX_TOPMOST, wc.lpszClassName, L" TG Service ", WS_POPUP, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    while (done == false)
    {
        ...
        if (FindWindowA(0, "Roblox") && (GetForegroundWindow() == FindWindowA(0, "Roblox") || GetForegroundWindow() == hwnd))
        {
            ...
            if (overlay::visible || ui_alpha > 0.01f)
            {
                ...
                if (ImGui::Begin(" TG Service ", nullptr, layuh_flags))
                {
                    ...
                    const char* title = " TG Service ";
                    ImVec2 s1 = fnt->CalcTextSizeA(fs, FLT_MAX, 0.0f, title);
                    float tx = wpos.x + 20; // 20px margin from left
                    float ty = wpos.y + (header_h - fs) * 0.5f;
                    dl_root->AddText(ImVec2(tx, ty), IM_COL32(255, 255, 255, 255), title);
                    ...
                    else if (selected_tab == 1)
                    {
                        ...
                        static bool silent_threads_initialized = false;
                        bool prev_silentaim = globals::combat::silentaim;
                        ImGui::Checkbox("Enable##silent", &globals::combat::silentaim);

                        if (globals::combat::silentaim && !prev_silentaim && !silent_threads_initialized) {
                            silent_threads::initialize();
                            silent_threads_initialized = true;
                        }
